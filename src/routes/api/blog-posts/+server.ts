...
export const GET: RequestHandler = async ({ url }) => {
	try {
		...
		return json({ error: 'Failed to load blog posts' }, { status: 500 });
	}
};
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!Array.isArray(categories)) {
			return json({ error: 'Categories must be an array' }, { status: 400 });
		}
		...
		return json({ error: 'Failed to load categorized blog posts' }, { status: 500 });
	}
};
